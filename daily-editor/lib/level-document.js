...
(function (root, factory) {
    ...
    class LevelDocument {
        ...
        applyTypeIdsOnly(assignmentById) {

            if (!assignmentById || typeof assignmentById !== 'object') return;

            for (const [idStr, typeId] of Object.entries(assignmentById)) {

                const id = Number(idStr);

                if (!Number.isFinite(id)) continue;

                if (this.editedById.has(id)) {

                    const patch = this.editedById.get(id);

                    this.editedById.set(id, { ...patch, typeId });

                    continue;

                }

                const added = this.addedTiles.find((t) => t.id === id);
                ...
                const raw = (this.raw.tiles ?? []).find((t) => t.id === id);

                if (raw && isDailyTile(raw) && !this.deletedIds.has(id)) {

                    this.editedById.set(id, {
